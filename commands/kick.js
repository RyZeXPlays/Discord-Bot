module.exports = {
    ...
    async execute(message, args){
        ...
        if (!message.member.permissions.has('KICK_MEMBERS')) {
            return message.reply('Perdon, pero no tienes el permiso para kickear personas')
        }

        let persona = message.mentions.members.first() || message.guild.members.resolve(args[0])

        if (!persona) {
            return message.reply('Debe mencionar a alguien para kickear')
        } else if(!persona.kickable){
            return message.reply('No puedo kickear a esta persona')
        }else if (persona.roles.highest.comparePositionTo(message.member.roles.highest) > 0) {
            return message.reply('Esta persona esta en la misma o mayor nivel de jerarquia que tu, no puedes kikcearlo')
        }
